...
# !/usr/bin/env python
# -*- coding: utf-8 -*-
"""For ease of working with hierarchical data, MTH is trying out using a YAML configuration file.
...
def get_default_config_filename():
    ...
    If PEYOTL_CONFIG_FILE is in the env that is the preferred choice;
    otherwise ~/.opentreeoflife/peyotl.yaml is preferred.
    If the preferred file does not exist, then the packaged peyotl/default_peyotl.yaml from
    the installation of peyotl is used.
    ...
    with _CONFIG_FN_LOCK:
        ...
        if not os.path.isfile(cfn):
            ...
            cfn = resource_filename(pr, 'peyotl/default_peyotl.yaml')
        if not os.path.isfile(cfn):
...
def get_raw_default_config_and_read_file_list():
    ...
    with _CONFIG_LOCK:
        ...
        if cfn:
            ...
            try:
                cfg = yaml.load(instream)
            except Exception as x:
                m = 'Error parsing configuration file "{}"'.format(cfn)
                logger(__name__).exception(m)
                raise RuntimeError('{} parsing exception message: {}'.format(m, str(x)))
            _CONFIG, _READ_DEFAULT_FILES = cfg, [cfn]
        else:
            _CONFIG, _READ_DEFAULT_FILES = {}, []
        return _CONFIG, _READ_DEFAULT_FILES
...
def _warn_missing_setting(config_filename, warn_on_none_level, missing_index, setting_list):
    if warn_on_none_level is None:
        return
    msg = _missing_setting_message(config_filename, missing_index, setting_list)
    logger(__name__).log(warn_on_none_level, msg)


def get_missing_ind_or_value_from_nested_dict(d, key_list):
    ind = 0
    try:
        for ind, s in enumerate(key_list):
            d = d[s]
        return None, d
    except:
        return ind, None
...
class ConfigWrapper(object):
    ...
    def _get_from_raw_dict_unchecked(self,
        ...
        if (default is None) and warn_on_none_level is not None:
            _warn_missing_setting(self._config_filename, warn_on_none_level, missing_ind, key_list)
        return default
...
def merge_into_dict_recursive(dest, overrides, out, higher, cfg_name):
    sk = set(dest.keys())
    kl = list(overrides.keys())
    kl.sort()
    for k in kl:
        dv = dest.get(k)
        n = higher + [k]
        if not isinstance(dv, dict):
            ...
            merge_into_dict_recursive(dest[k], overrides[k], out, n, cfg_name)
    sl = list(sk)
    sl.sort()
    for k in sl:
        v = dest[k]
...
def reverse_line_reader_gen(filename, buf_size=8192):

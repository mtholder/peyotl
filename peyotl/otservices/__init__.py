...
def launch_otcws(cfg):
    ott_dir = cfg.get_config_setting('ott', 'directory', raise_on_none=True)
    if not os.path.isdir(ott_dir):
        raise RuntimeError('[ott]directory "{}" is not a directory'.format(ott_dir))
    synth_par = cfg.get_config_setting('synthpar', 'directory', raise_on_none=True)
    if not os.path.isdir(synth_par):
        raise RuntimeError('[synthpar]directory "{}" is not a directory'.format(synth_par))
    dir = os.path.join(opentree_config_dir()[0], OTC_TOL_WS)
    port_num = cfg.get_config_setting('services', 'otcws_port', 1984)
    num_threads = cfg.get_config_setting('services', 'otcws_num_threads', 1)
    invoc = ['otc-tol-ws',

...
class Direct2OptimalNexson(NexsonConverter):
    ...
    def convert_tree(self, tree):
        ...
        if self.remove_old_structs:
            ...
            try:
                ...
                #       supporting this.
                m = 'Tree with ID "{}" is being dropped because it has no edges'.format(tid)
                logger(__name__).warn(m)
    ...
    def convert(self, obj):
        ...
        nex['@nexml2json'] = to_nexml2json(_BY_ID_HONEY_BADGERFISH)
        # Make the struct leaner
        if self.remove_old_structs:
            del nex['otus']
            del nex['trees']

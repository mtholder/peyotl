...
from .helper import (NexsonConverter,
                     get_nexml_el,
                     _DIRECT_HONEY_BADGERFISH,
                     NexsonError)
from ..phylo_syntax import to_nexml2json
...
class Optimal2DirectNexson(NexsonConverter):
    ...
    def convert_trees(self, trees_by_id, trees_el_order):
        ...
        tree_id_set = set()
        trees_id_set = set()
        for tgid in trees_el_order:
            # _LOG.debug('tgid = ' + tgid)
            tree_group = trees_by_id[tgid]
    ...
    def convert(self, obj):
        ...
        nex['trees'] = trees
        nex['@nexml2json'] = to_nexml2json(_DIRECT_HONEY_BADGERFISH)
